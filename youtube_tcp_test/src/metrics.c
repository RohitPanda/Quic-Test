		...
		if(metric.TSnow - metric.TS0 >= minbuffer || end)
		{
			...
			else
			{
				if (program_arguments.instantaneous_output)
					printf("youtubeevent12;%ld;%ld;%" PRIu64 ";%.3f\n",(long)gettimeshort(),(long)metric.htime/1000000,
                           metric.TS0, (double)(metric.Tmin - metric.Tmin0) / 1000);
				++metric.numofstalls;
...
void printvalues()
{
	...
	printf("YOUTUBE.6;%ld;%s;", (long)metric.htime/1000000,result);
	...
	printf("%.0f;", metric.tcp_connection_time_us[STREAM_VIDEO] * 1000 * 1000);
	printf("%.0f;", metric.full_ssl_connection_time_us[STREAM_VIDEO] * 1000 * 1000);
	...
	printf("%.0f;", metric.tcp_connection_time_us[STREAM_AUDIO] * 1000 * 1000);
	printf("%.0f;", metric.full_ssl_connection_time_us[STREAM_AUDIO] * 1000 * 1000);
	printf("%d;",metric.url[STREAM_AUDIO].bitrate);
	printf("%.0f;", metric.tcp_first_connection_time_us * 1000 * 1000);
	printf("%.0f;", metric.first_full_connection_time_us * 1000 * 1000);
