...
static bool check_connection_debug_output(const char* text, bool* connection_debug_out)
{
    if(strcmp(text, "--debug")==0)
    {
        *connection_debug_out=true;
        return true;
    }
    return false;
}

static bool check_steam_per_connection(const char* text, bool* one_stream_per_conn_out)
{
    if(strcmp(text, "--single_stream")==0)
    {
        *one_stream_per_conn_out=true;
        return true;
    }
    return false;
}
...
static bool check_port_numbers(int* current_index_ref, char **argv, uint** ports_ref, unsigned char* ports_count_out)
{
    if(strcmp(argv[*current_index_ref], "--p")==0)
    {
        /*value must be greater than LEN_CHUNK_MINIMUM seconds*/
        char* ports_string = argv[++(*current_index_ref)];
        *ports_count_out = (unsigned char) parse_array(ports_string, ports_ref);
        return true;
    }
    return false;
}

static bool check_quic_version(int* current_index_ref, char **argv, char* quic_version)
{
    if(strcmp(argv[*current_index_ref], "--qver")==0)
    {
        // Q039, Q042 - 4 symbols
        strncpy(quic_version, argv[++(*current_index_ref)], 4);
        return true;
    }
    return false;
}
...
int set_arguments(int argc, char **argv, char *youtubelink, struct program_arguments* arguments_out)
{
    for(int i=1; i<argc; i++)
    {
        ...
        if (check_instantaneous_output(argv[i], &arguments_out->instantaneous_output))
            continue;
        if (check_steam_per_connection(argv[i], &arguments_out->one_stream_per_connection))
            continue;
        if (check_max_bitrate(&i, argv, &arguments_out->max_bitrate))
            continue;
        if (check_buffer_len(&i, argv, &arguments_out->buffer_len))
            continue;
        if (check_quic_version(&i, argv, arguments_out->quic_version))
            continue;
        if (check_connection_debug_output(argv[i], &arguments_out->is_connection_level_debug))
            continue;
        if (check_port_numbers(&i, argv, &arguments_out->port_numbers, &arguments_out->ports_count))
            continue;
