...
typedef struct
{
	...
	char http_ver[NUMOFSTREAMS][SHORT_STRING_LEN]; // HTTP/1.1 for example
	char quic_ver[NUMOFSTREAMS][SHORT_STRING_LEN]; // QO39 for example
	...
	int errorcode;
	double connection_time_s[NUMOFSTREAMS];
	double first_website_connection_time_s;
	bool fail_on_stall;
	int playout_buffer_seconds;
} metrics;

struct quic_engine_shell* quic_client;
