...
if [ "" == "$PKG_OK" ]; then
  ...
  wget https://cmake.org/files/v3.10/cmake-3.10.3.tar.gz || exit -1
...
if [ "" == "$PKG_OK" ]; then
  ...
  wget --no-check-certificate https://www.tortall.net/projects/yasm/releases/yasm-1.2.0.tar.gz || exit -1
...
wget --no-check-certificate "https://curl.haxx.se/download/curl-${CURL_VERSION}.tar.gz" || exit -1
...
wget --no-check-certificate "https://ffmpeg.org/releases/ffmpeg-${FFMPEG_VERSION}.tar.gz" || exit -1
...
if [ "" == "$PKG_OK" ]; then
  ...
  wget --no-check-certificate https://github.com/libevent/libevent/releases/download/release-2.1.8-stable/libevent-2.1.8-stable.tar.gz || exit -1
...
wget --no-check-certificate "https://www.caida.org/tools/measurement/scamper/code/scamper-cvs-20180504.tar.gz" || exit -1
